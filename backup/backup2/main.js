...
if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
        ...
        contactForm.querySelectorAll('input[name="project"]:checked').forEach(cb => {
            ...
            formData.projects.push(label ? label.textContent : cb.value);
        });

        // Speichern im LocalStorage (Funktioniert ohne Server!)
        try {
            const existingRequests = JSON.parse(localStorage.getItem('beva_requests') || '[]');
            existingRequests.unshift(formData); // Neueste zuerst
            localStorage.setItem('beva_requests', JSON.stringify(existingRequests));
        } catch (e) {
